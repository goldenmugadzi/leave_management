...
class DashboardFilter extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      // Editing state
      editingCell: null,
      // {table: 'weekly_collections', row: 0, field: 'zwl_millions'} or {table: 'weekly_revenue_lost', row: 0, field: 'faults_mwh'} or {table: 'debtors', row: 0, field: 'percentage'} or {type: 'metric', field: 'energy_sold_value'}
      editingValue: "",
      originalValue: "",
      // Error handling
      error: null,
  ...
  initializeData = async () => {
    try {
      ...
      await Promise.all([this.getRegions(), this.getDashboardData(), this.getUserPermissions()]);
  ...
  getUserPermissions = () => {
    return new Promise((resolve, reject) => {
      ...
      fetch(`${BASE_URL}/dashboards/user_permissions`).then(response => {
        if (!response.ok) {
  ...
  onFilterSelectCenters = (filterType, event) => {
    ...
    if (filterType === "region") {
      ...
      if (value) {
        const filteredDistricts = this.state.allDistricts.filter(district => district.region_id == value);
        this.setState({
      ...
      if (value) {
        const filteredSections = this.state.allSections.filter(section => section.district_id == value);
        this.setState({
  ...
  showValidationHint = (message, type = 'info') => {
    ...
    if (editingInput) {
      ...
      hint.className = `absolute z-10 mt-1 p-2 text-xs rounded shadow-lg max-w-xs ${type === 'error' ? 'bg-red-100 text-red-800 border border-red-300' : type === 'warning' ? 'bg-yellow-100 text-yellow-800 border border-yellow-300' : 'bg-blue-100 text-blue-800 border border-blue-300'}`;
  ...
  saveEdit = () => {
    ...
    if (editingCell.type === 'metric') {
      ...
      } else if (field === 'outages' || field === 'resolved' || field === 'pending' || field === 'faults' || field === 'maintenance' || field === 'completed') {
  ...
  showPercentageAdjustmentFeedback = adjustments => {
    ...
    const adjustmentDetails = adjustments.filter(adj => Math.abs(adj.change) > 0.01) // Only show significant changes
    .map(adj => {
  ...
  validatePercentageInput = (value, currentRowIndex, debtorData) => {
    ...
    if (numValue + otherTotal > 100.1) {
      // Allow small buffer for auto-adjustment
      return {
        isValid: true,
        // Still valid, but will trigger auto-adjustment
  ...
  validateNewDataSections = data => {
    const sections = [{
      key: 'weekly_collections',
      name: 'Weekly Collections'
    }, {
      key: 'weekly_revenue_lost',
      name: 'Weekly Revenue Lost'
    }, {
      key: 'debtors',
      name: 'Debtors'
    }];
  ...
  formatAllDashboardData = dashboardData => {
    ...
    if (formatted.weekly_collections && Array.isArray(formatted.weekly_collections)) {
      formatted.weekly_collections = formatted.weekly_collections.map(item => this.formatWeeklyCollectionsData(item));
    }

    // Format weekly revenue lost data
    if (formatted.weekly_revenue_lost && Array.isArray(formatted.weekly_revenue_lost)) {
      formatted.weekly_revenue_lost = formatted.weekly_revenue_lost.map(item => this.formatWeeklyRevenueLostData(item));
    }

    // Format debtor category data
    if (formatted.debtors && Array.isArray(formatted.debtors)) {
      formatted.debtors = formatted.debtors.map(item => this.formatDebtorCategoryData(item));
    }
    return formatted;
  };
  ...
  renderEditableCell = (table, rowIndex, field, value, className = "") => {
    ...
    const cellClass = isReadOnlyField ? 'non-editable-cell auto-calculated' : canEdit ? 'editable-cell' : '';
    const tooltipText = canEdit && !isReadOnlyField ? "Click to edit" : isReadOnlyField ? "Auto-calculated field" : "";
    return /*#__PURE__*/React.createElement("td", {
  ...
  render() {
    ...
    if (this.state.error && this.state.networkError) {
      ...
      }, "Unable to Load Dashboard"), /*#__PURE__*/React.createElement("p", null, this.state.error), /*#__PURE__*/React.createElement("button", {
    ...
    return /*#__PURE__*/React.createElement("div", null, console.log("Current state:", {
      ...
      className: "dashboard-grid grid grid-cols-3 gap-4 mt-10"
    }, /*#__PURE__*/React.createElement("div", {
      className: "dashboard-section weekly-collections-section"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sm:flex lg:items-center lg:justify-between"
    }, /*#__PURE__*/React.createElement("div", {
      className: "min-w-0 flex-1"
    ...
    }, "\uD83D\uDCB0 Weekly Collections"), /*#__PURE__*/React.createElement("div", {
      style: {
        height: "14rem"
      },
      className: "mt-2 h-20 overflow-auto table-container"
    }, /*#__PURE__*/React.createElement("a", {
      href: "#"
      ...
      className: "text-left p-2 font-bold border border-gulf-blue-600"
    }, "ZWL (M)"), /*#__PURE__*/React.createElement("th", {
      className: "text-left p-2 font-bold border border-gulf-blue-600"
    }, "USD (M)"))), /*#__PURE__*/React.createElement("tbody", {
      className: "bg-white"
    }, this.state.weekly_collections && this.state.weekly_collections.length > 0 ? this.state.weekly_collections.map((collection, index) => /*#__PURE__*/React.createElement("tr", {
      key: index,
    ...
    }, collection.week), this.renderEditableCell('weekly_collections', index, 'zwl_millions', this.formatCurrencyMillions(collection.zwl_millions), 'text-gray-800 font-medium'), this.renderEditableCell('weekly_collections', index, 'usd_millions', this.formatCurrencyMillions(collection.usd_millions), 'text-gray-800 font-medium'))) : /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
      colSpan: "3",
      className: "p-2 text-center text-gray-500 border border-gray-300"
    }, "No weekly collections data available"))))))))), /*#__PURE__*/React.createElement("div", {
      className: "dashboard-section weekly-revenue-lost-section"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sm:flex lg:items-center lg:justify-between"
    }, /*#__PURE__*/React.createElement("div", {
      className: "min-w-0 flex-1"
      ...
      className: "dashboard-section-header"
    }, "\u26A1 Weekly Revenue Lost"), /*#__PURE__*/React.createElement("div", {
      style: {
        height: "14rem"
      },
      className: "mt-2 h-20 overflow-auto table-container"
      ...
      className: "text-left p-2 font-bold border border-red-600"
    }, "Faults (MWh)"), /*#__PURE__*/React.createElement("th", {
      className: "text-left p-2 font-bold border border-red-600"
    }, "Maintenance (MWh)"), /*#__PURE__*/React.createElement("th", {
      className: "text-left p-2 font-bold border border-red-600"
    }, "Total (MWh)"))), /*#__PURE__*/React.createElement("tbody", {
      className: "bg-white"
    }, this.state.weekly_revenue_lost && this.state.weekly_revenue_lost.length > 0 ? this.state.weekly_revenue_lost.map((revenueLost, index) => /*#__PURE__*/React.createElement("tr", {
      key: index,
      className: "hover:bg-red-50 transition-colors"
    }, /*#__PURE__*/React.createElement("td", {
      className: "p-2 font-semibold text-red-900 border border-gray-300"
    }, revenueLost.week), this.renderEditableCell('weekly_revenue_lost', index, 'faults_mwh', this.formatMWhWithDecimals(revenueLost.faults_mwh), 'text-gray-800 font-medium'), this.renderEditableCell('weekly_revenue_lost', index, 'maintenance_mwh', this.formatMWhWithDecimals(revenueLost.maintenance_mwh), 'text-gray-800 font-medium'), /*#__PURE__*/React.createElement("td", {
      className: "p-2 border border-gray-300 text-blue-700 font-medium bg-gray-50",
      title: "Auto-calculated total"
    }, this.formatMWhWithDecimals(revenueLost.total_mwh)))) : /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
      colSpan: "4",
      className: "p-2 text-center text-gray-500 border border-gray-300"
    }, "No weekly revenue lost data available"))))))))), /*#__PURE__*/React.createElement("div", {
      className: "dashboard-section debtors-section"
    }, /*#__PURE__*/React.createElement("div", {
      className: "sm:flex lg:items-center lg:justify-between"
    }, /*#__PURE__*/React.createElement("div", {
      className: "min-w-0 flex-1"
      ...
      className: "dashboard-section-header"
    }, "\uD83D\uDCCA Debtors by Category"), /*#__PURE__*/React.createElement("div", {
      style: {
        height: "14rem"
      },
      className: "mt-2 h-20 overflow-auto table-container"
      ...
      className: "bg-white"
    }, this.state.debtors && this.state.debtors.length > 0 ? this.state.debtors.map((debtor, index) => /*#__PURE__*/React.createElement("tr", {
      key: index,
      className: "hover:bg-purple-50 transition-colors"
    }, /*#__PURE__*/React.createElement("td", {
      className: "p-2 font-semibold text-purple-900 border border-gray-300"
    }, debtor.id || index + 1), /*#__PURE__*/React.createElement("td", {
      className: "p-2 font-semibold text-gray-800 border border-gray-300"
    }, debtor.category), this.renderEditableCell('debtors', index, 'percentage', this.formatPercentageWithSymbol(debtor.percentage), 'text-blue-700 font-medium'))) : /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
      colSpan: "3",
      className: "p-2 text-center text-gray-500 border border-gray-300"
