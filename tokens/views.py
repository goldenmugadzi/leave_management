...
def awaiting_my_action(request):
    ...
    for token in tokens:
        ...
        if token.process.workflow.step_set.filter(step=next_step, approver__in=user_roles).exists() and not token.process.approval_set.filter(approved="Rejected").exists():
