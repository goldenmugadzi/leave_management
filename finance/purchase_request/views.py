...
def purchase_request_detail(request, purchase_request_id):
    ...
    return render(request, 'finance/purchase_request/purchase_request_detail.html',
                  {'purchase_request': purchase_request, 'approved_steps': approved_steps, 'approvalForm': approvalForm,
                   'to': to})
...
def purchase_request_update(request, purchase_request_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'finance/purchase_request/create_purchase_request.html',
                          {'formset': itemFormset(instance=purchase_request), 'form': form})
    else:
...
def quote_purchase_request(request, purchase_request_id):
    ...
    if request.method == 'POST' and not request.POST.get('quote'):
        ...
        quote = QuotationForm()
        return render(request, 'finance/purchase_request/create_quote.html',
                      {'formset': itemFormset(initial=initial_data), 'quote': quote})
