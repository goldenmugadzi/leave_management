...
def set_appraisal_scoring_stage_completed(sender, instance, created, **kwargs):
    if not created:
        ...
        try:
            ...
            logger.info(f"[Approval Workflow stage] Scoring stage for Performance Dimension pk: {pk} handler initialized ...")
            ...
            if not score_qr.exists():
                logger.error(f"[Approval Workflow stage] Scoring stage for Performance Dimension pk: {pk}, Scoring objects not found")
                return None
            
            unscored_activity_score_qr = score_qr.filter(is_scored=False)
            if unscored_activity_score_qr.exists():
                logger.info(f"[Approval Workflow stage] Scoring stage for Performance Dimension pk: {pk}, Scoring objects not scored exists")
                return None
            ...
            if not workflow_qr.exists():
                logger.error(f"[Approval Workflow stage] Scoring stage for Performance Dimension pk: {pk}, All stages not found")
                return None            
            
            workflow_scoring_qr = workflow_qr.filter(stage_name=ApprovalStageData.scoring.value)
            if not workflow_scoring_qr.exists():
                logger.error(f"[Approval Workflow stage] Scoring stage for Performance Dimension pk: {pk}, {ApprovalStageData.scoring.value} stage  not found")
                return None
            
            workflow_repo.update(workflow_object=workflow_scoring_qr.first(), is_completed=True, updated_by=appraisal_object.appraiser)
            
            logger.success(f"[Approval Workflow stage] Scoring stage for Performance Dimension pk: {pk} completed")
        except Exception as e:
            logger.error(f"[Approval Workflow stage] Scoring stage for Performance Dimension pk: {pk}, failed with error: {e}")
            return None
